...
For example,     
Christova and Leader pointed out~\cite{Christova:1999he, Christova:2000nz} that at the leading order, 
...
is predicted to be very small~\cite{DSSV2008}.
...
Fig.~\ref{fig:polubardbar}  shows the HERMES and COMPASS result of $x(\Delta\bar{u} -
\Delta\bar{d})$ together with model predictions of a broken SU(2)$_f$
symmetry,  The data are
...
at a level appropriate to the sea contribution~\cite{Christova:1999he,Christova:2000nz}.
...
the NLO cross section has been well-known~\cite{DSSV2008}. 
%%-------------------------------------------------------------------------------
\begin{figure}[htbp] 
...
in which ${\cal C}$ and $\Delta C$ are well-known Wilson coefficients~\cite{DSSV2008}: 
...
coefficients already calculated~\cite{DSSV2008}. 
...
will serve as stringent constraints on pPDFs through NLO global fits, constraining sea quark polarization, and indirectly constrain gluon polarization $\Delta g(X)$, as shown through an earlier study for the JLab-6GeV case~\cite{epjcxj2006}.  
...
terms in the fragmentation functions are also expected to be largely canceled~\cite{Christova:1999he, Christova:2000nz}.
...
not contribute at any QCD-order~\cite{Christova:1999he, Christova:2000nz}.
...
are related to $\sigma^{\pi^+} - \sigma^{\pi^-}$. Since the gluon-related terms  
  are identical for  
$\pi^+$ and $\pi^-$ production, they drop out in the differences~\cite{Christova:1999he,Christova:2000nz}:
 \begin{eqnarray}
\label{Eq:a1pnlo}
...
as demonstrated in calculation of DSSV group~\cite{DSSV2008}.
...
$W^\pm$ decays.  Since the $Q^2$-evolutions of valence densities  $\Delta q_v$ 
...
was suggested~\cite{Christova:1999he, Christova:2000nz}  which requires prior knowledge of neither fragmentation functions nor
...
%\end{figure} 
%%------------------------------------------------------------------------------
The HERMES experiment extracted the combined asymmetry $A_{1N}^{h + \bar{h}}$ and shown to agree with the inclusive asymmetry $A_{1N}$.
